...
class Classroom:
    ...
    def insert_teacher_data(self,name,student_id):
        """"
        Args:
            name ([String])
        Returns:
            [Count the data size]
        """
        teacher = TeacherData(teacher_name=name, student_id=student_id)
        session.add(teacher)
        session.commit()
    ...
    def update_row_teacher(self, value):
            ...
            """
            try:
                session.query(TeacherData).filter(TeacherData.id < 10).update(
                    {TeacherData.name: value})
                session.commit()
                return session.query(TeacherData.name).one()
            except Exception as e:
                logger.exception(e)
    ...
    def delete_row_teacher(self):
        ...
        """
        try:
            session.query(TeacherData).filter(TeacherData.id < 10).delete()
            session.commit()
            return session.query(TeacherData).count()
        except Exception as e:
            logger.exception(e)